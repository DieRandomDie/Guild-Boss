...
2. Run **initialSetup** once and authorize spreadsheet access. It saves the spreadsheet ID and creates an empty `Data` sheet if needed. Existing row 1 must contain exactly one each of `Remaining`, `Minutes`, and `Seconds` (case-sensitive, any order). No Date column is required or written.
...
4. Copy the web app's **/exec** URL into `URL` in `gboss.user.js` if it differs from the existing URL. Saving Apps Script source alone does not update a versioned deployment.
